...
/**
 * Lightweight Groq REST API client — v2
...
object GroqClient {
    ...
    fun chat(
        ...
        return try {
            conn.apply {
                ...
                setRequestProperty("User-Agent", "DiscordCreatorV2/2.0")
                doOutput = true
    ...
    fun generateUsernames(apiKey: String, count: Int): List<String> {
        val result = chat(
            apiKey,
            systemPrompt = "You are a Discord username generator. Output ONLY the requested items, one per line, nothing else. No numbering, no extra text.",
            userPrompt = """Generate exactly $count unique Discord usernames.
Rules:
- Lowercase letters, digits and underscores ONLY
- Length 4-20 characters
- Gamer/internet culture style (e.g. shadow_wolf_42, neon_blade_99)
...
- Mix of styles: some camelCase, some plain words, some with numbers
- Human-looking, creative, gamer-themed
- No special symbols except spaces and apostrophes
- Each on its own line""",
            maxTokens = count * 20
        )
        if (result.error.isNotBlank()) return emptyList()
...
An hCaptcha or similar CAPTCHA is blocking me. Give me a single short tip (1-2 sentences) on how to solve it manually. Be direct and concise.""",
            maxTokens = 80,
            temperature = 0.3
        )
