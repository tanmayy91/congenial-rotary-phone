...
object AccountManager {
    ...
    /**
     * Generate a mixed-case alphanumeric string (used inside email suffixes).
     * Occasional special characters make the alias look more human-typed.
     */
    ...
    /**
     * Generate a Gmail alias from base email.
     * e.g. tanmayyy911@gmail.com → tanmayyy911+xK3pQ7@gmail.com
     * Length varies (7–12 chars) and mixes case to look hand-typed.
     */
    fun generateEmailAlias(baseEmail: String, index: Int): String {
        ...
        val length = 7 + (index % 6)          // 7–12 chars
        val suffix = humanSuffix(length)
        return "$localPart+$suffix$domain"
    }

    /**
     * Generate a human-looking display name, e.g. "PixelDragon" or "DarkWolf".
     * No underscores/numbers – suitable for Discord's Global Name field.
     */
    fun generateDisplayName(index: Int): String {
        val adjectives = listOf(
            ...
            "Rogue", "Silent", "Turbo", "Vivid", "Wild", "Xenon", "Zero"
        )
        val nouns = listOf(
    ...
    /**
     * Generate a plausible Discord username (unique handle).
     * Format: lowercase_word_number, 4–20 chars.
     * Uses a shuffled pick so consecutive usernames look unrelated.
     */
    ...
    /**
     * Build list of AccountInfo objects to create.
     * Each gets a unique email alias, display name, username, and random DOB.
     * Optional [groqUsernames] and [groqDisplayNames] lists override local generation.
     * Birth years target the 18–28 year-old demographic (realistic ages).
     */
    ...
    /**
     * Append account entry to acc.txt and tokens.txt in the app's files dir.
     * acc.txt format: email:password:token
     * tokens.txt format: token
     */
    ...
    /**
     * Read full contents of tokens.txt.
     */
    ...
    /**
     * Clear both output files (fresh run).
     */
